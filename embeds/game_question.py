...
class GameQuestion(Embed):
    def __init__(self, question_id, number, quantity,
                 closed=True, chosen=None, **kwargs):
        ...
        if closed:
            self.keyboard = [
                [
                    Button(label=f'{i + 1}',
                           custom_id=f'answer_options:'
                                     f'{question_id},{number},{quantity},{i}')
